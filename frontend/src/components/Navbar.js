    ...
    <header className="w-full h-20 border-b border-gray-100 bg-white sticky top-0 z-50 flex items-center justify-center shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
      <div className="w-full max-w-[1400px] px-6 lg:px-12 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          {!loading && user ? (
            <>
              <Link href="/dashboard" className="text-[14px] font-medium text-gray-600 hover:text-black transition-colors">
                Dashboard
              </Link>
              <button onClick={logout} className="bg-black hover:bg-gray-800 text-white px-5 py-2.5 rounded-md text-[14px] font-medium transition-transform hover:-translate-y-0.5 shadow-md">
                Logout
              </button>
            </>
          ) : (
